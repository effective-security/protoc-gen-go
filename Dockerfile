...
RUN curl -L https://github.com/protocolbuffers/protobuf/releases/download/v35.1/protoc-35.1-linux-x86_64.zip -o /tmp/protoc.zip
RUN unzip /tmp/protoc.zip -d /tmp/protoc3
RUN mv /tmp/protoc3/bin/* /usr/local/bin/
RUN cp -r /tmp/protoc3/include/ /usr/local/

RUN curl -L https://github.com/protocolbuffers/protobuf-javascript/releases/download/v4.0.2/protobuf-javascript-4.0.2-linux-x86_64.zip -o /tmp/protobuf-javascript.zip
...
RUN curl -L https://go.dev/dl/go1.26.5.linux-amd64.tar.gz -o /tmp/go.tar.gz
